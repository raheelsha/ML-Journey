# 🤖 ML Journey — Raheel Shaukat

A documented journey of learning Machine Learning and AI from scratch, 
built during my internship at **NetSol Technologies**.

## 📚 Weekly Progress

### Week 1 — Python & Data Fundamentals
- ✅ Day 1: Python Basics (Variables, Lists, Dictionaries, Loops)
- ✅ Day 2: Functions & Object Oriented Programming
- ✅ Day 3: NumPy & List Comprehensions
- ✅ Day 4: Pandas & Data Manipulation
- ✅ Day 5: Data Visualization (Matplotlib & Seaborn)
- ✅ Mini Project: Titanic Dataset Analysis

## 🛠️ Tools & Technologies
- Python 3.11
- NumPy & Pandas
- Matplotlib & Seaborn
- Scikit-learn (coming soon)
- PyTorch (coming soon)
- Hugging Face Transformers (coming soon)

## 🚀 Projects
| Project | Description | Status |
|---------|-------------|--------|
| Titanic Analysis | Exploratory Data Analysis on Titanic dataset | ✅ Complete |
| NLP Web App | Sentiment analysis web application | 🔄 Coming Soon |
| RAG Chatbot | Document Q&A chatbot using LLMs | 🔄 Coming Soon |

## 📈 Goals
- Master Python for AI/ML
- Build strong foundation in Machine Learning
- Specialize in NLP & Large Language Models
- Land a job as AI/ML Engineer

## 👨‍💻 About Me
- 🎓 Computer Science Student
- 💼 AI/ML Intern at NetSol Technologies
- 📍 Lahore, Pakistan
- 🔗 [LinkedIn](www.linkedin.com/in/raheel-shaukat-b93a79304)
